// TestMode — layout giống quizlet-clone test-form (tất cả câu trên một trang)
...
export function TestMode({ cards, studySetId }: Props) {
  ...
  async function submitTest() {
    ...
    const payload: QuizAnswer[] = questions.map((item) => ({
      ...
      selectedIndex: answers[item.flashcardId]?.selectedIndex,
      attempts: answers[item.flashcardId]?.attempts ?? 1,
      responseTimeMs: answers[item.flashcardId]?.responseTimeMs ?? 0,
    }));
    try {
  ...
  function choose(flashcardId: number, choice: string, selectedIndex: number) {
  ...
  if (result) {
    ...
    return (
      <div className="learn-done">
        ...
        <p className="learn-score"><strong>{result.score}</strong> / {result.total} ({pct}%)</p>
        ...
        <div className="learn-review">
          {questions.map((item) => {
            const r = result.cardResults.find((x) => x.flashcardId === item.flashcardId);
            return (
              <div key={item.flashcardId} className={`review-row ${r?.correct ? "correct" : "wrong"}`}>
                <span className="review-term">{item.term}</span>
                <span className="review-answer">{answers[item.flashcardId]?.answer || "—"}</span>
                <span className="review-correct">{r?.correct ? "Đúng" : "Sai"}</span>
              </div>
            );
          })}
        </div>
          ...
          return (
            <section key={q.flashcardId} className="test-question-block">
              <div className="test-question">
                <p className="test-question-label">Câu {qIndex + 1} · Chọn định nghĩa đúng</p>
                <p className="test-term">{q.term}</p>
              </div>
              <div className="test-choices">
                {q.choices.map((choice, i) => (
                  <button
                    key={`${q.flashcardId}-${choice}-${i}`}
                    type="button"
                    className={`test-choice${selected?.selectedIndex === i ? " selected" : ""}`}
                    onClick={() => choose(q.flashcardId, choice, i)}
                    disabled={Boolean(selected) || submitted}
                    aria-label={`Câu ${qIndex + 1}, đáp án ${String.fromCharCode(65 + i)}: ${choice}`}
                  >
...
function toQuestions(items: QuizGeneratedItem[]) {
  return items
    .filter((item) => item.kind === "question" || item.choices)
    .map((item) => ({
      flashcardId: item.flashcardId,
      term: item.term ?? item.text ?? "",
