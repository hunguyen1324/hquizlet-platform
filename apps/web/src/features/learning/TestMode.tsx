...
function buildQuestions(cards: Flashcard[]): TestQuestion[] {
  ...
  return shuffleArray(cards).map((card) => {
    const distractors = shuffleArray(allDefs.filter((d) => d !== card.definition)).slice(0, 3);
    return { card, choices: shuffleArray([card.definition, ...distractors]), userAnswer: null, correct: null };
  });
}
      ...
      <div className="learn-done">
        ...
        <div className="learn-review">
          ...
          {state.questions.map((item, i) => (
            <div key={i} className={`review-row ${item.correct ? "correct" : "wrong"}`}>
              <span className="review-term">{item.card.term}</span>
              <span className="review-answer">{item.userAnswer || "—"}</span>
              <span className="review-correct">{item.card.definition}</span>
            </div>
          ))}
        </div>
